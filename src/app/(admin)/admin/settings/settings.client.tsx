...
import { CheckCircle, AlertCircle, Eye, EyeOff, Key, Trash2, ExternalLink } from 'lucide-react'
import { saveGoogleKeysAction, disableGoogleAction } from './actions'
...
export default function SettingsClient({ configured, clientIdMasked }: Props) {
  ...
  const [maskedId, setMaskedId] = useState(clientIdMasked)
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      const result = await saveGoogleKeysAction(clientId, clientSecret)
      if (result.success) {
        setIsConfigured(true)
        setClientId('')
        setClientSecret('')
        if (result.error) {
          setStatus({ type: 'warning', msg: result.error })
        } else {
          setStatus({ type: 'success', msg: 'Google OAuth configured successfully.' })
          // Re-mask the ID for display
          setMaskedId(clientId.length > 12 ? `${clientId.slice(0, 8)}••••${clientId.slice(-4)}` : '••••••••••••')
        }
      } else {
        setStatus({ type: 'error', msg: result.error ?? 'Failed to save keys.' })
      }
    } catch {
  ...
  async function handleDisable() {
    if (!confirm('Disable Google sign-in? Users will no longer be able to sign in with Google.')) return
    setStatus(null)
    setDisabling(true)
    try {
      const result = await disableGoogleAction()
      if (result.success) {
        setIsConfigured(false)
        setMaskedId(null)
        setStatus({ type: 'success', msg: 'Google OAuth disabled.' })
      } else {
        setStatus({ type: 'error', msg: result.error ?? 'Failed to disable.' })
      }
    } catch {
      setStatus({ type: 'error', msg: 'An unexpected error occurred.' })
    } finally {
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-8">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-slate-500">Manage integrations and OAuth providers.</p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="px-6 py-5">
          ...
          {isConfigured && maskedId && (
            <div className="mb-5 rounded-lg border border-slate-100 bg-slate-50 px-4 py-3">
              ...
              <p className="mt-1 font-mono text-sm text-slate-700">{maskedId}</p>
            </div>
          )}
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="flex items-center justify-between pt-1">
              ...
              >
                <Key size={14} />
                {saving ? 'Saving...' : isConfigured ? 'Update Keys' : 'Save & Enable'}
              </button>
            </div>
          </form>
        </div>
        ...
        <div className="rounded-b-2xl border-t border-slate-100 bg-slate-50 px-6 py-4">
          ...
          <ol className="mt-2 space-y-1 text-xs text-slate-400">
            ...
            <li>3. Add <code className="rounded bg-slate-200 px-1 py-0.5 font-mono text-slate-600">{process.env.NEXT_PUBLIC_SITE_URL ?? 'http://localhost:3000'}/api/auth/oauth/callback</code> as an authorized redirect URI</li>
            <li>4. Paste your Client ID and Client Secret above</li>
          </ol>
        </div>
      </div>
    </div>
